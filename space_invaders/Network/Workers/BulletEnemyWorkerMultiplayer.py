...
class BulletEnemyWorkerMultiplayerThread(QThread):
    update_enemy_bullet = pyqtSignal(Bullet)
    finish_enemy_shooting = pyqtSignal()

    def __init__(self, screen: QWidget, enemies: list, enemy_bullets: list, current_level: int, bullet_id: str):
        super().__init__()
        ...
        self.bullet = BulletFactory.create_object(self.screen, f"{self.bullet_id}", self.random_enemy.x + 10,
